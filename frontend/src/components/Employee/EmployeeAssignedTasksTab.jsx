...
const EmployeeAssignedTasksTab = () => {
  ...
  return (
    <div className="relative grid grid-cols-1 gap-10 z-40
      place-items-center
      sm:place-items-stretch
      sm:grid-cols-[repeat(auto-fit,minmax(300px,1fr))]
      ">
        {employeeTasksData?.length===0 && (
          <p className='text-xl sm:text-2xl font-bold text-center'>Sit back and Relax, No Tasks Assigned for you </p>
        )}
      {employeeTasksData &&
        employeeTasksData.map((emp) => (
          <div key={emp._id} className="relative sm:w-[350px] sm:h-[250px] w-[280px] h-[200px] shadow-md shadow-black/60 rounded-md bg-[#fefae0]/50 p-3">
            <div className="flex flex-col items-start flex-1 gap-2">
              <div className="flex w-full  items-center justify-between">
                <p className="rounded-md px-3 py-1 bg-[#023047] text-white text-sm">
                  {formatDate(emp.createdAt)}
                </p>
                <div className={`w-[20px] h-[20px] ${emp.status==="pending"? 'bg-[#c1121f]':'bg-green-600'} rounded-full`}></div>
              </div>
...
export default EmployeeAssignedTasksTab;
